...
describe('wallet_updateAccount', () => {
  test.runIf(Anvil.enabled)('default', async () => {
    ...
    porto_newAccount._internal.store.setState(porto._internal.store.getState())
...
describe('wallet_sendCalls', () => {
  ...
  test.runIf(Anvil.enabled)('behavior: `feeToken` capability', async () => {
    const porto = getPorto()
    const client = TestConfig.getServerClient(porto)
    const contracts = TestConfig.getContracts(porto)
  ...
  test('behavior: merchant fee sponsor', async () => {
    const porto = getPorto()
    const client = TestConfig.getServerClient(porto)
    const contracts = TestConfig.getContracts(porto)
  ...
  test.runIf(!Anvil.enabled)('behavior: required funds (address)', async () => {
    const porto = getPorto()
    const client = TestConfig.getServerClient(porto)
    const contracts = TestConfig.getContracts(porto)
  ...
  test.runIf(!Anvil.enabled)('behavior: required funds (symbol)', async () => {
    const porto = getPorto()
    const client = TestConfig.getServerClient(porto)
    const contracts = TestConfig.getContracts(porto)
...
test('behavior: unsupported wallet_ method', async () => {
  ...
  ).rejects.toThrowError('The provider does not support the requested method.')
})
