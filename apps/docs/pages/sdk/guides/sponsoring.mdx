...
### Set up Merchant RPC Route

Next, we will set up our Merchant RPC Route at a `/merchant` endpoint.
...
There are usages for each of the frameworks in the [`Route.merchant` API reference](/sdk/api/route/merchant).
...
const porto = await Porto.create({
  merchantRpcUrl: 'https://myapi.com/merchant'
})
```
...
That's it! Once you have a server set up at the `/merchant` path of your app (or another remote URL), Porto will automatically
